...
public class OrganizerMessageManager extends MessageManager {
    ...
    public boolean sendAllMessageEvent(Event event, String messageContent){
        for (attendee:event.attendee) {
            sendSingleMessage(attendee.userName, messageContent);
        }
        return true
    }
    ...
    public boolean sendSingleMessageEvent(Event event, String messageContent, String receiverID){
        if (event.attendee.contains(receiverID)) {
     ...
     * @return true
     */
    public void sendAllMessageSpeaker(Event event, String messageContent, ArrayList<Speaker> speakers){
        for (speakers) {
            sendSingleMessage(attendee.userName, messageContent);
        }
        return true;
